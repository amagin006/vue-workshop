...
#### やること
フォームから受け取ったデータを使って`totalAmount`, `incomeTotal`, `expensesTotal`を計算する。  
※ヒント: 収支タイプに気をつけてください。
...
ブラウザで確認すると、コンソールにallItemが所持するIncomeとExpensesの配列データが表示されるはずです。
ここまで来たら後もう少し！ 残るはこのallItemが所持する配列データを最下部のリストに表示する事です。

完成に向けてラストスパートといきましょう！
