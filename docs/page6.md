...
#### 8-1-2. $emit
子コンポーネントは適当なタイミングで(殆どの場合methodsの中で実行されます。)  
 `$emit` を使って親から渡されたイベント（Budgetyの場合は`submit-handler`）を実行します。  
...
フォームに入力された値や選択値と`data`プロパティに指定した変数を結びつけます。  
`v-model`ディレクティブはデータバインディングとイベントハンドリング２つの処理をまとめ、完結に記述するための物です。  
その為フォーム側では`value`や`checked`などの属性にバインディングされます。  

`v-model`を使わずにこれを行う場合、

1. データをバインディングする  
   DOM要素にデータバインドする事でデータの変更を検出し紐付いたDOMを自動的に更新します。
1. イベントハンドリングする
   フォームの`<input>`の様にユーザーからの入力を受け付けるDOM要素では、ユーザーの入力イベントをトリガーにデータを取得します。  
1. イベントハンドラの中でデータバインドされたデータを更新する

通常であればこう言った流れで行う双方向データバインディングを`v-model`がまとめて担い自動化する事で、  
より簡潔に処理を行える訳です。  
なお`v-model`で受け取るデータ型は通常入力フォームであればすべて文字列に、複数選択できるようなフォームではデータ型は配列になります。しかし、あたいにデータバインディングを使用した場合（もともと用意されているデータの中から選択するフォームなどの場合）、データ型はバインドされているデータによって変わります。

```vue
<!-- v-modelはv-bindとv-onで置き換えられる -->
<input
